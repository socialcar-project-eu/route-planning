...
namespace SocialCar.RoutePlanner.Containers.RTree
{
    ...
    public class RTree<T>
    {
        ...
        {
            Stack<int> parentsLocal = new Stack<int>();
            Stack<int> parentsEntryLocal = new Stack<int>();
            ...
            parentsLocal.Clear();
            parentsLocal.Push(rootNodeId);

            parentsEntryLocal.Clear();
            parentsEntryLocal.Push(-1);
            Node<T> n = null;
            int foundIndex = -1;  // index of entry to be deleted in leaf

            while (foundIndex == -1 && parentsLocal.Count > 0)
            {
                n = getNode(parentsLocal.Peek());
                int startIndex = parentsEntryLocal.Peek() + 1;
                ...
                if (!n.isLeaf())
                {
                    ...
                    for (int i = startIndex; i < n.entryCount; i++)
                    {
                        ...
                        {
                            parentsLocal.Push(n.ids[i]);
                            parentsEntryLocal.Pop();
                            parentsEntryLocal.Push(i); // this becomes the start index when the child has been searched
                            parentsEntryLocal.Push(-1);
                            contains = true;
            ...
            if (foundIndex != -1)
            {
                ...
                condenseTree(n, parentsLocal, parentsEntryLocal);
                msize--;
            }
        ...
        private void contains(Rectangle r, intproc v)
        {
            ...
            while (parentsLocal.Count > 0)
            {
                Node<T> n = getNode(parentsLocal.Peek());
                int startIndex = parentsEntryLocal.Peek() + 1;
        ...
        */
        ...
        private void condenseTree(Node<T> l, Stack<int> parentsLocal, Stack<int> parentsEntryLocal)
        {
            ...
            {
                parent = getNode(parentsLocal.Pop());
                parentEntry = parentsEntryLocal.Pop();
